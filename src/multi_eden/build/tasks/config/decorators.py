...
def resolve_config_env(config_env: Optional[str], args: Tuple, kwargs: dict, 
                      task_name: str, default_env_callback: Optional[Callable] = None) -> str:
    """
    ...
    # Set the configuration environment in the core config system
    try:
...
def requires_config_env(func: Callable) -> Callable:
    ...
    4. Fail early with a strong-typed exception if the environment cannot be loaded
    
    Usage:
        @task
        @requires_config_env
        def my_task(ctx, config_env=None):
    ...
    def wrapper(ctx, *args, **kwargs):
        ...
        resolved_env = resolve_config_env(config_env, args, kwargs, task_name, None)
